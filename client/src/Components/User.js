...
export default function User() {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                ...
                const res = await axios.get(`http://localhost:3001/users/${decoded.id}`, {
    ...
    const handlePlaceOrder = async () => {
        try {
            ...
            await axios.post('http://localhost:3001/orders', orderPayload, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const removeFromCart = (itemId) => {
        axios.post('http://localhost:3001/removefromcart', { itemId }, {
            headers: { Authorization: `Bearer ${token}` }
        }).then(response => {
        ...
        const fetchUserAndOrders = async () => {
            try {
                ...
                const [userRes, ordersRes] = await Promise.all([
                    axios.get(`http://localhost:3001/users/${decoded.id}`, {
                        headers: { Authorization: `Bearer ${token}` }
                    }),
                    axios.get(`http://localhost:3001/orders/${decoded.id}`, {
                        headers: { Authorization: `Bearer ${token}` }
                    })
                ]);
...
const cancelOrder = async (orderId) => {
    if (window.confirm("Are you sure you want to cancel this order?")) {
        try {
            await axios.put(`http://localhost:3001/orders/${orderId}/cancel`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
            alert("Order cancelled.");
            const updated = await axios.get(`http://localhost:3001/orders/${user._id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
